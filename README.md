...
- **修复**：QQ 机器人断线后不再永久掉线 —— 此前 QQ 网关正常关闭连接（`wss closed with code 1000`）
  后 SDK 不重连、机器人静默下线直到重启服务器；现在开启任意关闭码自动重连（3 秒后重新鉴权恢复会话），
  并新增常驻连接看门狗兜底（持续断开超过 1 分钟时记录日志并触发重连）
- **修复**：所有报错（渲染失败、图片发送失败、指令处理异常等）此前只进控制台、
  日志文件里查不到的问题 —— 插件全部警告/错误现在双写控制台与插件日志文件
  `logs/qq/qq-bind-日期.log`，卡片生成失败还会附带堆栈前几帧，事后可完整追溯
- **修复**：图片生成失败的一类根因 —— 超大尺寸背景照片（如手机原图）整图解码
  导致内存峰值 / OOM；背景图改为子采样解码（最长边限制约 1600px），
  解码失败的坏图自动进入 10 分钟负缓存不再抽中
- **调整**：信息卡片统计项从 15 项精简为 4 项：**金币、称号、在线时长、今日在线时长**
  （2 列 × 2 行布局，卡片高度收紧为 660px，观感更紧凑）
- **修复**：称号无法检测的问题 —— 默认接入 **DeluxeTags**（反射调用，无需硬依赖；
  在线玩家取当前激活标签，离线玩家回退到 `userdata/player_tags.yml` 持久化的选择；
  未安装 DeluxeTags 时仍回退 Vault 前缀 → 权限组）。称号中的颜色符号
  （`&`/`§` 颜色码、`&#RRGGBB` 十六进制色）会被解析并按原色渲染到卡片上，
  过暗颜色自动提亮保证深色背景可读，超长称号自动截断
- **新增**：图片发送失败时回复文本提示（与“卡片生成失败”区分，便于定位是
  机器人连接问题还是渲染问题）

### v0.1.5.4 更新日志

- **修复**：0.1.5.3 打包缺陷（JAR 目录条目缺失）导致 QQ 机器人无法连接、命令无响应的问题
  （SDK 组件扫描依赖目录条目；打包脚本已固化"必须保留目录条目"规则）
- **修复**：机器人连接过程日志（初始化 → 鉴权 → 组件装配 → WebSocket 连接成功/超时/失败）
  此前仅在控制台输出，现在同步写入插件日志文件 `logs/qq/qq-bind-日期.log`，事后可追溯
- **调整**：数据保存日志不再显示在控制台，改为仅写入插件日志文件（同上路径），控制台保持清爽

### v0.1.5.3 更新日志

- **修复**：信息卡片背景图改为从 `img/` 目录中**随机挑选**一张（旧版固定取排序后第一张），
  多张背景图每次渲染随机切换；每张图的处理结果按文件签名缓存，随机切换不增加 CPU 开销
- **新增**：数据保存日志 —— 启动装载、每次定时/退出保存均记录一行汇总
  （玩家数 + 耗时 + 文件大小），异步执行、不产生额外磁盘写，对性能几乎零影响
